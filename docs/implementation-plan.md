...
The September 10, 2026 release covers bilingual CH-01–CH-04 following explicit publication authorization. It produces no PDF and makes no PDF-layout investment. The September 13, 2026 authorization additionally releases all completed CH-05–CH-39 and Primer-PY language pairs. On September 15, the initial CH-54–CH-57 publication was followed by explicit authorization to publish every completed chapter and Primer-HTTP. All 57 core chapters and both primers are now available. The incomplete multimodal extension remains excluded. Future additions require their own review and authorization. Public documentation and code comments are English; the book and its associated localized content remain bilingual. See CONTRIBUTING.md for the editorial contract.

The following increment descriptions record earlier draft authoring stages; their original validation records remain historical evidence. Current publication scope is recorded in `docs/releases/2026-09-15-complete-core.md`, supplementing the earlier releases, including the initial September 15 Part IX release. Publication does not rewrite historical draft statements or measured hashes.
...
## Part VII draft increment

CH-40–46 and Primer-HTTP extend the same knowledge assistant through context/output contracts, a shared read-only Tool/HTTP/MCP operation, retained lexical/dense comparisons, cited extractive answers and layered RAG evaluation. The eight shared figures and all sixteen language pages are static drafts. The HTTP primer reuses the figure renderer without acquiring chapter numbers. Existing published pages and KA-0/KA-1 evidence remain unchanged.

`pnpm ci:build` now includes `validate:part-vii` and `test:part-vii:cpu`. Build environments must install the pinned runtime subset in `code/part-vii/requirements-ci.txt`; the book and main-site build workflows declare this step. The fuller practice requirements support two fresh-kernel notebooks through `pnpm notebooks:part-vii`. The shared browser entry includes all sixteen pages in both viewports, figure themes, language switching and no-JavaScript access. Results, editorial repairs and the deliberately unmeasured LLM/production scope are recorded in `docs/part-vii-validation.md`; no release follows from this increment.

## Part VIII increment (2026-09-14 working draft)

Chapters 47–53 extend the existing assistant with a one-choice state workflow, source-verification Skill, local approved writes with durable recovery, layered system evaluation, human-reference Judge calibration, and governance/security fixtures. Chapter IDs/order and published Part I remain unchanged; the Chinese draft titles of Chapters 47 and 52 are synchronized with the configuration. No additional Primer is needed: the actual Python and HTTP Primer implementations supply the programming and transport prerequisites.

The content blueprint, primary-source record and actual verification record are `docs/part-viii-blueprint.md`, `docs/part-viii-research.md` and `docs/part-viii-validation.md`. Runtime snapshot details are separate in `docs/part-viii-online.md`. New verification commands join existing checks rather than replace them. No deployment or publication is authorized by this increment.

## Part IX increment (2026-09-15 working draft)

Chapters 54–57 complete KA-9 and KA-10 within a declared local scope: a CPU service around the existing extractive RAG, structured contract, source Skill and tenant-aware read store; explicit cancellation, bounded work, citations and usable feedback; manifest-bound candidate gates, trace/replay distinctions, local canary and rollback; cache, breaker, SLO and reviewed regression evidence. The capstone keeps the immutable lexical comparisons, failed model selection and known synonym miss. The earlier approved-write recovery and read-only MCP transport are integrated as separately scoped evidence, not exposed as new web authority.

The eight manuscripts and four shared bilingual figures reuse the existing static renderer, headings, math and draft filtering. The complete HTTP Primer remains draft and provides the practice prerequisite; Chapter 54 adds the job-lifecycle bridge. No new teaching controls, PDF work, hosting migration or publication is introduced. Draft Chinese titles are synchronized with configuration without changing IDs/order.

`validate:part-ix` and `test:part-ix:cpu` join `ci:build`; `records:part-ix` and `notebooks:part-ix` explicitly regenerate current evidence and fresh-kernel outputs. The browser entry covers all eight pages on desktop/mobile, both figure themes, language navigation and no-JavaScript reading, plus the live service UI. Blueprint, primary sources, actual validation, dated implementation details, capstone evaluation/cards/licenses and the run/rollback handbook are linked from `docs/part-ix-capstone.md`. Existing published chapters remain published and all previous draft work is preserved.

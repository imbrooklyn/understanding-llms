...
const ids=Array.from({length:7},(_,i)=>`ch-${47+i}`);
const evidence:Record<string,string[]>={
 'ch-47':['use_evidence','ABSTAINED','750','RNN'], 'ch-48':['SKILL.md','1.0.0','225'],
 'ch-49':['0.35','UNCERTAIN','A-104'], 'ch-50':['3/4','0/4','8/8'],
 'ch-51':['750','600','Data Card'], 'ch-52':['DEMO-CABINET-42','send_http','&lt;img'],
 'ch-53':['PermissionError','orders:read:own','0/4']
};
...
for(const locale of ['en','zh-hans']) for(const id of ids){
 test(`${locale}/${id}: Part VIII complete static evidence and localized figures`,async({page,request},info)=>{
  ...
  if(id==='ch-50') await body.locator('table').nth(2).screenshot({path:info.outputPath('complete-rankings.png'),style});
  if(id==='ch-53') await body.locator('table').nth(2).screenshot({path:info.outputPath('layered-evaluation.png'),style});
...
for(const locale of ['en','zh-hans']) test(`${locale}: all Part VIII evidence without JavaScript`,async({browser},info)=>{

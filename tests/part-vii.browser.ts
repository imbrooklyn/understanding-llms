...
const ids=[...Array.from({length:7},(_,i)=>`ch-${40+i}`),'primer-http'];
const evidence:Record<string,string[]>={
  'ch-40':['1,200','ka2-context-v1'], 'ch-41':['ka-answer-v1','additionalProperties','max_tokens'],
  'ch-42':['A-104','B-205','orders:read:own'], 'ch-43':['2026-07-28','clientCapabilities','structuredContent'],
  'ch-44':['1.574906','0.173765','0.953866'], 'ch-45':['591','348','local-extractive-v1'],
  'ch-46':['7/12','8/12','144','562'], 'primer-http':['BOOK_DEMO_KEY','Retry-After','0.02']
};
...
for(const locale of ['en','zh-hans']) for(const id of ids){
 test(`${locale}/${id}: Part VII complete static evidence and localized figures`,async({page,request},info)=>{
  ...
  if(id==='ch-44') await body.locator('table').nth(2).screenshot({path:info.outputPath('complete-rankings.png'),style});
  if(id==='ch-46') await body.locator('table').nth(2).screenshot({path:info.outputPath('layered-evaluation.png'),style});
...
for(const locale of ['en','zh-hans']) test(`${locale}: all Part VII evidence without JavaScript`,async({browser},info)=>{
